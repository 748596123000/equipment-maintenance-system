...
>(({ className, ...props }, ref) => {
  ...
  return (
    <DialogPrimitive.Overlay
      ...
      style={{ backgroundColor: theme === 'light' ? 'rgba(0, 0, 0, 0.4)' : 'rgba(0, 0, 0, 0.6)' }}
      aria-hidden="true"
      {...props}
    />
  )
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        style={{
          background: isLight ? colors.cardBg : colors.cardBg,
          border: `1px solid ${colors.cardBorder}`,
          boxShadow: isLight
            ? '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)'
            : '0 25px 50px -12px rgba(0, 0, 0, 0.7), 0 0 40px rgba(0, 240, 255, 0.08)',
        }}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <DialogPrimitive.Close
          className={cn(
            "absolute right-4 top-4 rounded-full opacity-70 ring-offset-background transition-all hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none p-2"
          )}
          style={{
