...
class ComplianceCheckRequest(BaseModel):
    guide_content: str = Field(..., description="指引内容JSON字符串")
    task_description: str = Field(..., description="作业任务描述")
    equipment_type: Optional[str] = Field(default=None, description="设备类型")
    safety_level: str = Field(default="standard", description="安全等级")


@router.post("/compliance-check", summary="合规校验")
async def compliance_check(request: ComplianceCheckRequest):
    ...
    try:
        ...
        results = generator.check_compliance(
            guide=guide,
            task_description=request.task_description,
            equipment_type=request.equipment_type,
            safety_level=request.safety_level,
        ...
        return {
            ...
            "data": {
                ...
                "warning_checks": sum(1 for r in results if not r["passed"]),
            },
        }
    except Exception as e:
        logger.error(f"合规校验失败: {e}", exc_info=True)
