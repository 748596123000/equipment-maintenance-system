"""
视觉模型服务模块 - 麒麟V11 LoongArch适配版

适配说明：
- LoongArch架构上transformers+torch需要从源码编译，非常耗时
- 优先使用DashScope API（qwen-vl-max）进行图片理解
- 本地模型支持需手动安装torch/transformers并下载模型
- 设置HF_HUB_OFFLINE=1避免自动下载模型

支持后端：
- dashscope: DashScope qwen-vl API（推荐）
- local: 本地Qwen2-VL模型（需手动配置，LoongArch需源码编译依赖）
- unavailable: 无可用后端
"""
...
def get_vision_service() -> VisionServiceLoongArch:
    global _vision_instance
    with _vision_lock:
        if _vision_instance is None:
            _vision_instance = VisionServiceLoongArch()
