...
export default function SearchPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: '抱歉，图片检索失败，请稍后重试。' }])
    } finally {
      setLoading(false)
      setQuestion('')
      clearImage()
  ...
  return (
    <div className={`space-y-6 transition-all duration-700 ${isMounted ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <Tabs defaultValue="text" className="w-full">
        <TabsList className="h-12 p-1 mb-4 rounded-xl" style={{ 
          background: isLight ? '#f8fafc' : 'rgba(15, 15, 35, 0.8)', 
          border: `1px solid ${isLight ? '#e2e8f0' : colors.CYBER_CYAN + '15'}` 
        }}>
          <TabsTrigger value="text" className="h-10 px-6 text-sm font-medium rounded-lg transition-all duration-300 data-[state=active]:bg-white data-[state=active]:text-black dark:data-[state=active]:text-black">
            <Zap size={16} className="mr-2" />
            文本检索
          </TabsTrigger>
          <TabsTrigger value="image" className="h-10 px-6 text-sm font-medium rounded-lg transition-all duration-300 data-[state=active]:bg-white data-[state=active]:text-black dark:data-[state=active]:text-black">
            <Image size={16} className="mr-2" />
            图片检索
          </TabsTrigger>
        </TabsList>

        <TabsContent value="text" className="h-[calc(100vh-280px)]">
          <div className="glass-card h-full p-6" style={{ 
            background: isLight ? '#ffffff' : cardBg, 
            border: `1px solid ${isLight ? '#e2e8f0' : colors.CYBER_CYAN + '15'}` 
          }}>
            <Suspense fallback={<ComponentLoader />}>
            <ChatPanel sessionKey="search_text" title="知识检索" searchMode="hybrid" topK={5} />
          </Suspense>
          </div>
        </TabsContent>

        <TabsContent value="image" className="h-[calc(100vh-280px)]">
          <div className="glass-card h-full flex flex-col overflow-hidden" style={{ 
            background: isLight ? '#ffffff' : cardBg, 
            border: `1px solid ${isLight ? '#e2e8f0' : colors.CYBER_CYAN + '15'}` 
          }}>
            {/* Header - Theme-aware */}
            <div className="flex items-center justify-between p-4" style={{ borderBottom: `1px solid ${isLight ? '#e2e8f0' : colors.CYBER_CYAN + '15'}` }}>
              <div className="flex items-center gap-3">
                ...
                >
                  <Image size={20} style={{ color: isLight ? colors.CYBER_BLUE : '#000' }} />
                </div>
                <div>
                  <GradientText as="h2"
                  ...
                  <p className="text-xs" style={{ color: textSecondary }}>上传故障图片，AI 智能分析诊断</p>
                </div>
              </div>
              <Button
                variant="ghost"
                ...
                onMouseEnter={(e) => { e.currentTarget.style.color = accentColor; e.currentTarget.style.background = `${accentColor}10` }}
                ...
                <div className="flex h-full items-center justify-center">
                  <div className="text-center">
                    <div 
                      ...
                      style={{ 
                        background: isLight ? `${colors.CYBER_BLUE}10` : `${colors.CYBER_CYAN}15`, 
                        boxShadow: isLight ? `0 0 30px ${colors.CYBER_BLUE}15` : `0 0 30px ${colors.CYBER_CYAN}20` 
                      }}
                    >
                      <Sparkles size={40} style={{ color: accentColor }} />
                    </div>
                    <p className="text-lg font-medium mb-2" style={{ color: textPrimary }}>上传图片开始智能检索</p>
                    <p className="text-sm" style={{ color: textSecondary }}>支持 JPG、PNG、BMP、GIF、WebP 格式</p>
                  </div>
                </div>
              )}
                      ...
                      <div className="flex justify-start mt-2">
                        <div className="relative group">
                          <img src={msg.imageUrl} alt="上传图片" className="max-h-56 rounded-xl object-cover" style={{ 
                            ...
                            boxShadow: isLight ? '0 8px 32px rgba(0,0,0,0.08)' : `0 8px 32px rgba(0,240,255,0.15)` 
                          }} />
                          <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: `linear-gradient(to top, ${accentColor}20, transparent)` }} />
                        </div>
                      </div>
                    )}
                    {msg.role === 'assistant' && msg.sources && msg.sources.length > 0 && (
                      <div className="flex justify-start mt-2">
                        <div className="max-w-[80%]">
                          <SourceCard sources={msg.sources.map((s) => ({ source: s.title, content: s.content, score: s.score }))} />
                        </div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="max-w-[75%] rounded-2xl rounded-bl-sm px-5 py-3" style={{ 
                      ...
                      <div className="flex items-center gap-3">
                        <div 
                          ...
                          style={{ 
                            background: isLight ? `${colors.CYBER_BLUE}10` : `linear-gradient(135deg, ${colors.CYBER_CYAN} 0%, ${colors.CYBER_BLUE} 100%)` 
                          }}
                        >
                          <Sparkles size={16} style={{ color: isLight ? colors.CYBER_BLUE : '#000' }} />
                        </div>
                        <div className="flex items-center gap-1">
                          <div className="w-2 h-2 rounded-full animate-bounce" style={{ background: accentColor, animationDelay: '0s' }} />
                          <div className="w-2 h-2 rounded-full animate-bounce" style={{ background: accentColor, animationDelay: '0.2s' }} />
                          <div className="w-2 h-2 rounded-full animate-bounce" style={{ background: accentColor, animationDelay: '0.4s' }} />
                        </div>
                        <span className="text-sm" style={{ color: accentColor }}>正在分析图片并检索...</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {image && imagePreview && (
              <div className="px-4 py-3" style={{ borderTop: `1px solid ${isLight ? '#e2e8f0' : colors.CYBER_CYAN + '15'}` }}>
                <div className="flex items-center gap-3">
                  <div className="relative group">
                    <img src={imagePreview} alt="预览" className="h-16 w-16 rounded-xl object-cover" style={{ 
                      border: `1px solid ${accentColor}30`, 
                      boxShadow: isLight ? '0 4px 16px rgba(0,0,0,0.08)' : `0 4px 16px ${colors.CYBER_CYAN}20` 
                    }} />
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-sm font-medium" style={{ color: textPrimary }}>{image.name}</p>
                    <p className="text-xs" style={{ color: textSecondary }}>{(image.size / 1024).toFixed(1)} KB • 点击右侧按钮开始检索</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="p-4" style={{ borderTop: `1px solid ${isLight ? '#e2e8f0' : colors.CYBER_CYAN + '15'}` }}>
              {!image ? (
                <div
                  ...
                  style={{ 
                    border: `2px dashed ${isDragging ? accentColor : accentColor}40`,
                    background: isDragging ? `${accentColor}10` : (isLight ? '#f8fafc' : 'rgba(15, 15, 35, 0.5)')
                  }}
                ...
                >
                  <div 
                    ...
                    style={{ background: isLight ? `${colors.CYBER_BLUE}10` : `${colors.CYBER_CYAN}15` }}
                  >
                    <Upload className="h-7 w-7" style={{ color: accentColor }} />
                  </div>
                  <p className="text-sm font-medium mb-1" style={{ color: textPrimary }}>拖拽图片到此处，或点击选择图片</p>
                  <p className="text-xs" style={{ color: textSecondary }}>支持 JPG、PNG、BMP、GIF、WebP 格式</p>
                  <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleFileInput} />
                </div>
              ) : (
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 transition-colors pointer-events-none z-10" style={{ color: textSecondary }}>
                      <Sparkles size={18} />
                    </div>
                    <Input
                      value={question}
