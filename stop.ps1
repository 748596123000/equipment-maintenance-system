...
$streamlitProcs = Get-Process -Name python -ErrorAction SilentlyContinue | Where-Object {
    try {
        $cmdLine = (Get-CimInstance Win32_Process -Filter "ProcessId = $($_.Id)").CommandLine
        $cmdLine -match "streamlit"
    } catch { $false }
}

if ($streamlitProcs) {
    foreach ($proc in $streamlitProcs) {
        Write-Host "  停止 Streamlit 进程 PID=$($proc.Id)..." -ForegroundColor Yellow
        Stop-Process -Id $proc.Id -Force
        $stopped++
    }
    Write-Host "  Streamlit 已停止" -ForegroundColor Green
}
